...
function comboDelta(map, ev, now = Date.now()) {
  ...
  const open = (count) => {
    map.set(key, { last: repeat, counted: count, at: now, closed: !!ev.repeatEnd });
    prune(map, now);
  };
  ...
  if (prev.closed) {
    ...
    if (repeat > prev.counted) { const d = repeat - prev.counted; open(repeat); return d; }
  ...
  map.set(key, { last: repeat, counted, at: now, closed: !!ev.repeatEnd });
  prune(map, now);
  return delta;
}
